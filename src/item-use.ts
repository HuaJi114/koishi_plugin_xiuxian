...
import { getBaseMaxHpMp } from './utils'
...
export function getItemUseCategory(info: ItemInfo, back?: XiuxianBack): ItemUseCategory {
  const t = info.item_type ?? back?.goodsType ?? (info.type as string) ?? ''
  if (t === '法器' || t === '防具' || t === '装备') return 'equipment'
  if (t === '功法' || t === '神通' || t === '辅修功法' || t === '技能') return 'skill'
  if (t === '丹药' || t === '合成丹药') return 'elixir'
  if (t === '聚灵旗') return 'jlq'
  return 'other'
}

function goodsRankOf(info: ItemInfo): number {
  return Number(info.rank ?? 99999)
}

/** 玩家境界是否满足物品最低要求（USERRANK：数值越小境界越高） */
function meetsMinRealm(data: GameData, playerLevel: string, goodsRank: number): boolean {
  return data.itemRankByLevel(playerLevel) <= goodsRank
}

function realmName(data: GameData, info: ItemInfo): string {
  return (info['境界'] as string) || data.rankToLevelName(goodsRankOf(info))
}

/** 批量使用数量受每日/总耐药性限制 */
function bulkUseCheck(info: ItemInfo, useNum: number, dayNum: number, allNum: number): number {
  const dayLimit = Number(info.day_num ?? Number.MAX_SAFE_INTEGER)
  const allLimit = Number(info.all_num ?? Number.MAX_SAFE_INTEGER)
  let dayCan = useNum
  if (dayNum + useNum > dayLimit) dayCan = Math.max(dayLimit - dayNum, 0)
  let allCan = useNum
  if (allNum + useNum > allLimit) allCan = Math.max(allLimit - allNum, 0)
  return Math.min(dayCan, allCan, useNum)
}

export async function useBackItem(
  ctx: Context,
  srv: XiuxianService,
  userId: string,
  back: XiuxianBack,
...
async function useEquipment(
  ...
  if (back.state === 1) return '该装备已被装备，请勿重复装备！'
  ...
  if (type === '装备') {
    ...
    if (isArmor) await srv.equipArmor(userId, goodsId)
    else await srv.equipFaqi(userId, goodsId)
    return `成功装备${name}！`
  }
  return '无法识别该装备类型！'
}

async function useSkillBook(
  srv: XiuxianService,
  userId: string,
  back: XiuxianBack,
  info: ItemInfo,
...
async function useElixir(
  ...
  const { maxHp, maxMp } = getBaseMaxHpMp(player.exp)
  ...
  switch (buffType) {
    case 'level_up_rate': {
      ...
      if (actual <= 0) return `道友使用的丹药：${name}已达到使用上限！`
...
async function useJulingqi(
  ...
  const player = (await srv.getPlayer(userId))!
  if (!player.blessedSpotFlag) return '道友还未拥有洞天福地，无法使用该物品'
  const buff = await srv.getBuff(userId)
  const speed = Number(info['修炼速度'] ?? 0)
  if (speed <= 0) return '聚灵旗数据异常，无法使用！'
...
export async function unequipItem(
  ...
  if (back.state !== 1) return `【${info.name}】当前未装备！`
