...
import { findMixRecipes, MixRecipe, tiaohe, YaocaiEntry } from '../mix-elixir-util'

/** 用户上次炼丹查询到的配方列表 */
const pendingRecipes = new Map<string, MixRecipe[]>()

/** 炼丹模块（对照 nonebot xiuxian_mixelixir） */
export function applyMixElixir(ctx: Context, _config: Config) {
  ...
  ctx.command('xiuxian/炼丹', '扫描背包并列出可炼制丹方')
    .action(async ({ session }) => {
      ...
      if (!hasLdl) return '道友背包内没有炼丹炉，无法炼丹！'

      const yaocaiList: YaocaiEntry[] = []
      for (const b of backs) {
        const info = srv.data.getItem(b.goodsId)
        if (!info || info.item_type !== '药材') continue
        yaocaiList.push({ id: b.goodsId, info, num: b.goodsNum })
      }
      if (!yaocaiList.length) return '道友背包内没有药材，无法炼丹！'

      const mixItems = srv.data.getItemsByType(['合成丹药'])
      const recipes = findMixRecipes(yaocaiList, mixItems)
      if (!recipes.length) return '系统未检测到可用丹方，道友背包内的药材不满足配方或冷热失和！'
      ...
      if (recipes.length > 20) lines.push(`（仅显示前20条，共${recipes.length}条）`)
  ...
  ctx.command('xiuxian/炼制 <num:integer>', '按序号炼制丹药')
    .action(async ({ session }, num) => {
      const userId = session!.userId!
      if (!num || num < 1) return '请输入正确的配方序号！'
      const recipes = pendingRecipes.get(userId)
      if (!recipes?.length) return '请先发送【炼丹】查看可用配方！'
      const recipe = recipes[num - 1]
      if (!recipe) return '没有这个序号的配方，请重新【炼丹】查看列表！'

      const consume = async (goodsId: number, need: number): Promise<boolean> => {
      ...
      if (tiaohe(mainInfo, recipe.mainNum, yyInfo, recipe.yaoyinNum)) {
