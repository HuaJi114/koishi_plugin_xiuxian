...
import { numberTo, randChoice } from '../utils'
  ...
  ctx.command('xiuxian/凝丹', '消耗灵石凝练一枚随机丹药')
    .action(async ({ session }) => {
      ...
      if (!(await srv.costStoneForUser(userId, cost, pf))) {
        return `凝丹需消耗灵石${numberTo(cost)}枚，道友灵石不足！`
      }
      ...
      const chosen = randChoice(elixirIds)
      ...
      return `丹成！道友消耗灵石${numberTo(cost)}枚，凝练出一枚【${info.name}】！`
    })
}
