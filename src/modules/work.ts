...
const FREE_REFRESH = 3
const REFRESH_COST = 500000
const ABORT_COST = 4000000

/** 悬赏令模块：刷新、接取、结算、终止 */
export function applyWork(ctx: Context, _config: Config) {
  ...
  /** 已刷新但未接取的悬赏令列表（内存） */
  const pending = new Map<string, WorkOption[]>()
  /** 用户当日刷新次数 */
  const refreshCount = new Map<string, { date: string; count: number }>()

  function getRefresh(userId: string): number {
    const today = new Date().toDateString()
    const rec = refreshCount.get(userId)
    return !rec || rec.date !== today ? 0 : rec.count
  }
  function addRefresh(userId: string): void {
    const today = new Date().toDateString()
    const rec = refreshCount.get(userId)
    if (!rec || rec.date !== today) refreshCount.set(userId, { date: today, count: 1 })
    else rec.count += 1
  }

  /** 计算成功率，对应 workmake.countrate */
  function countRate(exp: number, needExp: number): [number, number] {
    let rate = Math.floor((exp / needExp) * 100)
    let isOut = 1
    if (rate >= 100) {
      let tp = 1
      let e = exp
      while (e / needExp * 100 > 100) { tp += 1; e /= 1.5 }
      rate = 100
  ...
  async function doRefresh(userId: string, platform: string, isManual: boolean): Promise<string> {
    const player = await srv.getPlayer(userId)
    if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
    const cd = await srv.getCd(userId)
    if (cd && cd.type === 1) return '已经在闭关中，请输入【出关】结束后才能获取悬赏令！'
    if (cd && cd.type === 3) return '道友在秘境中，请等待结束后才能获取悬赏令！'
    const [active] = await ctx.database.get('xiuxian_work', { userId })
    if (active) return `进行中的悬赏令【${active.workName}】，请输入【悬赏令结算】或【悬赏令终止】！`

    let used = getRefresh(userId)
    let stoneMsg = ''
    if (isManual && used >= FREE_REFRESH) {
    ...
    addRefresh(userId)
    const free = Math.max(FREE_REFRESH - getRefresh(userId), 0)
    ...
    lines.push(`(每日免费刷新${FREE_REFRESH}次，超出后每次消耗灵石${REFRESH_COST}，今日剩余免费次数：${free}次)${stoneMsg}`)
    return lines.join('\n')
  }
  ...
  ctx.command('xiuxian/悬赏令接取 <num:integer>', '接取指定编号的悬赏令')
    .action(async ({ session }, num) => {
      ...
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      const cd = await srv.getCd(userId)
      if (cd && cd.type !== 0) return '道友当前有事在身，无法接取悬赏令！'
      const list = pending.get(userId)
      if (!list) return '没有查到你的悬赏令信息呢，请刷新！'
      if (!num || num < 1 || num > list.length) return '请输入正确的任务序号'
  ...
  async function doSettle(userId: string, reward: 'exp' | 'stone'): Promise<string> {
    ...
    if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
    const [active] = await ctx.database.get('xiuxian_work', { userId })
    if (!active) return '没有查到你的悬赏令信息呢，请刷新！'
    ...
    const success = randInt(1, 100) <= active.rate
    ...
    if (success && active.itemId) {
      ...
      if (item) {
        await srv.sendBack(userId, active.itemId, item.name, (item.type as string) ?? (item.item_type as string) ?? '物品', 1)
