...
import {
  calcMissionReward,
  fightMissionMonsters,
  generateMissionMonsters,
  getMissionById,
  pickRandomMission,
...
function sectOfferingStone(position: number): number {
  const pos = Math.min(Math.max(position, 0), 4)
  if (pos === 4) return SECT_OFFERING_OUTER
  if (pos === 0) return SECT_OFFERING_LEADER
  const ratio = (4 - pos) / 4
...
export function applySect(ctx: Context, config: Config) {
  ...
  ctx.command('xiuxian/创建宗门 <name:text>', '创建一个宗门')
    .action(async ({ session }, name) => {
      ...
      name = (name ?? '').trim()
      if (!name) return '请输入宗门名称！'
      if (player.sectId) return '道友已有宗门，无法创建！'
      ...
      if (stone < config.sectCreateCost) return `创建宗门需要${config.sectCreateCost}灵石，道友灵石不足！`
      if (!(await srv.costStoneForUser(userId, config.sectCreateCost, pf))) return '灵石不足！'
  ...
  ctx.command('xiuxian/我的宗门', '查看所处宗门信息')
    ...
    .action(async ({ session }) => {
      ...
      if (!sect) return '宗门信息异常，你当前按散修处理。请【退出宗门】后重新加入。'
  ...
  ctx.command('xiuxian/退出宗门', '退出当前宗门')
    .action(async ({ session }) => {
      ...
      const player = await srv.getPlayer(userId)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId) return '道友尚未加入任何宗门！'
      if (player.sectPosition === 0) return '宗主无法直接退出宗门，请先【宗主传位】！'
  ...
  ctx.command('xiuxian/宗门捐献 <amount:integer>', '向宗门捐献灵石')
    .action(async ({ session }, amount) => {
      ...
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId) return '道友尚未加入任何宗门！'
      if (!amount || amount <= 0) return '请输入正确的捐献数量！'
      if (!(await srv.costStoneForUser(userId, amount, session!.platform))) return '道友的灵石不足！'
      const sect = (await srv.getSectById(player.sectId))!
  ...
  ctx.command('xiuxian/宗门职位变更 <position:integer>', '宗主调整成员职位（需 @成员）')
    .action(async ({ session }, position) => {
      const player = await srv.getPlayer(session!.userId!)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId || player.sectPosition !== 0) return '只有宗主才能变更职位！'
      const targetId = getAtId(session!)
      if (!targetId) return '请 @ 要变更职位的成员！'
      if (position < 1 || position > 4) return '职位编号需为 1-4（长老/亲传/内门/外门）！'
      const target = await srv.getPlayer(targetId)
      if (!target || target.sectId !== player.sectId) return '对方不是本宗门成员！'
  ...
  ctx.command('xiuxian/踢出宗门', '宗主移除成员（需 @成员）')
    .action(async ({ session }) => {
      const player = await srv.getPlayer(session!.userId!)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId || player.sectPosition !== 0) return '只有宗主才能踢人！'
      const targetId = getAtId(session!)
      if (!targetId) return '请 @ 要踢出的成员！'
      const target = await srv.getPlayer(targetId)
      if (!target || target.sectId !== player.sectId) return '对方不是本宗门成员！'
      if (targetId === session!.userId) return '宗主无法踢出自己！'
  ...
  ctx.command('xiuxian/宗主传位', '将宗主之位传给成员（需 @成员）')
    .action(async ({ session }) => {
      const player = await srv.getPlayer(session!.userId!)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId || player.sectPosition !== 0) return '只有宗主才能传位！'
      const targetId = getAtId(session!)
      if (!targetId) return '请 @ 要传位的成员！'
      const target = await srv.getPlayer(targetId)
      if (!target || target.sectId !== player.sectId) return '对方不是本宗门成员！'
  ...
  ctx.command('xiuxian/升级攻击修炼', '提升攻击修炼等级')
    .action(async ({ session }) => {
      const player = await srv.getPlayer(session!.userId!)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId) return '攻击修炼需要加入宗门后进行！'
      const sect = (await srv.getSectById(player.sectId))!
      const cap = Math.floor(sect.sectScale / 100000)
      if (player.atkPractice >= cap) return `当前宗门建设度仅支持攻击修炼至${cap}级，请提升宗门建设度！`
      const cost = (player.atkPractice + 1) * 1000000
      if (!(await srv.costStoneForUser(session!.userId!, cost, session!.platform))) return `升级攻击修炼需要${cost}灵石，道友灵石不足！`
  ...
  ctx.command('xiuxian/宗门每日供奉', '按宗门职位领取每日供奉')
    .action(async ({ session }) => {
      ...
      const player = await srv.getPlayer(userId)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId) return '散修无法领取宗门供奉，请先加入宗门。'
      if (player.sectOfferingGet === 1) return '今日供奉已领取，请明日0点后再来。'
      ...
      if (randInt(1, 100) <= pillRate) {
        ...
        if (itemId !== 0) {
          ...
          if (info) {
            await srv.sendBack(userId, Number(itemId), info.name, (info.item_type as string) ?? '丹药', 1)
  ...
  function sectMissionCdMessage(player: { sectMissionDoneAt?: Date | null }): string | undefined {
    if (!player.sectMissionDoneAt) return undefined
    const elapsed = dateDiffSeconds(new Date(), player.sectMissionDoneAt)
    if (elapsed >= SECT_MISSION_CD_SECONDS) return undefined
    const remainHours = Math.floor((SECT_MISSION_CD_SECONDS - elapsed) / 3600)
    const remainMins = Math.ceil(((SECT_MISSION_CD_SECONDS - elapsed) % 3600) / 60)
  ...
  ctx.command('xiuxian/接取宗门任务', '随机接取一项宗门任务')
    ...
    .action(async ({ session }) => {
      ...
      const player = await srv.getPlayer(userId)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId) return '散修无法接取宗门任务，请先加入宗门。'
      if (player.sectMissionId) return '道友已有进行中的宗门任务，请发送【完成宗门任务】执行，或【我的宗门任务】查看详情。'
      const cdMsg = sectMissionCdMessage(player)
      if (cdMsg) return cdMsg
      if (isHeavilyInjured(player.exp, player.hp)) {
      ...
      return [
        ...
        `提示：每位道友完成一次宗门任务后，需等待 ${SECT_MISSION_CD_SECONDS / 3600} 小时方可再次接取。`,
      ].join('\n')
    })
  ...
  ctx.command('xiuxian/我的宗门任务', '查看当前宗门任务')
    .action(async ({ session }) => {
      ...
      if (!player.sectMissionId) {
        const cdMsg = sectMissionCdMessage(player)
        if (cdMsg) return `当前没有进行中的宗门任务。\n${cdMsg}\n冷却结束后可发送【接取宗门任务】。`
        return '当前没有进行中的宗门任务。发送【接取宗门任务】随机接取一项。'
      }
      const mission = getMissionById(player.sectMissionId)
      if (!mission) return '任务数据异常，请重新【接取宗门任务】。'
      return [
  ...
  ctx.command('xiuxian/完成宗门任务', '执行已接取的宗门任务')
    .action(async ({ session }) => {
      ...
      const player = await srv.getPlayer(userId)
      if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
      if (!player.sectId) return '散修无法完成宗门任务，请先加入宗门。'
      if (!player.sectMissionId) {
      ...
      const reward = calcMissionReward(initialMonsterHp)
      const pf = session!.platform
