...
function stoneCultivateUsage(): string {
  return [
    '命令格式：灵石修炼 <灵石数量>',
...
/** 双修每日次数（内存计数，按日期重置） */
const twoExpCount = new Map<string, { date: string; count: number }>()
...
export function applyCultivate(ctx: Context, config: Config) {
  ...
  ctx.command('xiuxian/闭关', '进入闭关修炼状态')
    .action(async ({ session }) => {
      ...
      if (cd && cd.type !== 0) return stateMsg(cd.type)
  ...
  async function doOutClosing(userId: string, useStone: boolean): Promise<string> {
    const player = await srv.getPlayer(userId)
    if (!player) return '修仙界没有道友的信息，请输入【我要修仙】加入！'
    const cd = await srv.getCd(userId)
    if (!cd || cd.type !== 1) return '道友现在并没有在闭关哦~'
  ...
  ctx.command('xiuxian/灵石修炼 <amount:integer>', '消耗灵石直接转化修为')
    ...
    .action(async ({ session }, amount) => {
      ...
      if (amount > stone) return stoneCultivateInsufficient(amount, stone)
      ...
      if (exp >= userGetExpMax) {
        ...
        if (cost > stone) return stoneCultivateInsufficient(cost, stone)
        await srv.addExp(userId, userGetExpMax)
  ...
  ctx.command('xiuxian/双修', '与道侣一同修炼（需 @ 对方）')
    .action(async ({ session }) => {
      ...
      if (!targetId) return '请 @ 你的道侣，与其一起双修！'
      if (targetId === session!.userId) return '道友无法与自己双修！'
      const target = await srv.getPlayer(targetId)
      if (!target) return '修仙者应一心向道，务要留恋凡人！'
      if (target.exp > player.exp) return '修仙大能看了看你，不屑一顾，扬长而去！'
      if (getTwoExpCount(player.userId) >= TWO_EXP_LIMIT) return '道友今天双修次数已经到达上限！'
      if (getTwoExpCount(target.userId) >= TWO_EXP_LIMIT) return '对方今天双修次数已经到达上限！'

      const gain = (p: typeof player): number => {
      ...
      if ([13, 14, 52, 10, 66].includes(randInt(1, 100))) {
  ...
  function stateMsg(type: number): string {
    if (type === 1) return '道友现在在闭关呢，小心走火入魔！'
    if (type === 2) return '道友现在在做悬赏令呢，小心走火入魔！'
    if (type === 3) return '道友现在正在秘境中，分身乏术！'
    return '道友现在什么都没干呢~'
  }
}
