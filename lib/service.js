...
class XiuxianService extends koishi_1.Service {
    ...
    async getRealPlayer(userId) {
        ...
        const merged = (0, skills_1.mergeSkillBuffs)(skills, this.data);
        const stats = (0, combat_stats_1.computeCombatStats)(player, buff, this.data, merged);
    ...
    async wipeAndRemake(userId, platform) {
        const player = await this.getPlayer(userId);
        if (!player)
    ...
    async ramake(userId, _root, _rootType) {
        const player = await this.getPlayer(userId);
        if (!player)
            return '修仙界没有你的足迹，输入 我要修仙 加入修仙世界吧！';
    ...
    async resetDailyFlags() {
        ...
        await this.setMeta(daily_utils_1.META_LAST_DAILY_RESET, (0, daily_utils_1.todayStr)());
    }
    /** 启动或跨日时补执行每日重置 */
    async ensureDailyResetIfNeeded() {
        const today = (0, daily_utils_1.todayStr)();
        const last = await this.getMeta(daily_utils_1.META_LAST_DAILY_RESET);
        if (last === today)
            return;
    ...
    async hasSkill(userId, skillId) {
        const [row] = await this.ctx.database.get('xiuxian_skill', { userId, skillId });
        return !!row;
    }
    async learnSkill(userId, skillId, skillType) {
        ...
        const buff = await this.getBuff(userId);
        const patch = {};
        if (skillType === '功法' && !buff.mainBuff)
            patch.mainBuff = skillId;
        if (skillType === '神通' && !buff.secBuff)
            patch.secBuff = skillId;
        if (skillType === '辅修功法' && !buff.subBuff)
            patch.subBuff = skillId;
        if (Object.keys(patch).length) {
            await this.ctx.database.set('xiuxian_buff', { userId }, patch);
        }
    }
    async getSecSkillIds(userId) {
        const skills = await this.getLearnedSkills(userId);
        return skills.filter((s) => s.skillType === '神通').map((s) => s.skillId);
    }
    /** 将旧 buff 单字段迁移到 skill 表 */
    async migrateSkillsFromBuff() {
        const FLAG = 'skill_table_migrated_v1';
        if (await this.getMeta(FLAG))
            return;
        const buffs = await this.ctx.database.get('xiuxian_buff', {});
        for (const b of buffs) {
            if (b.mainBuff > 0)
                await this.learnSkill(b.userId, b.mainBuff, '功法');
            if (b.secBuff > 0)
                await this.learnSkill(b.userId, b.secBuff, '神通');
            if (b.subBuff > 0)
                await this.learnSkill(b.userId, b.subBuff, '辅修功法');
        }
