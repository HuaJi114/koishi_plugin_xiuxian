...
function applyInfo(ctx, _config) {
    ...
    ctx.command('xiuxian/我的修仙信息', '查看角色存档')
        ...
        const stats = (0, combat_stats_1.computeCombatStats)(base, buff, srv.data);
        ...
        return [
            ...
            `突破状态：${expMsg}概率：${rate}%`,
            `攻击力：${(0, utils_1.numberTo)(player.atk)}（${(0, combat_stats_1.formatAtkBreakdown)(stats)}）`,
    ...
    ctx.command('xiuxian/我的状态', '查看当前气血与真元')
        ...
        const lines = [
            ...
            `攻击：${(0, utils_1.numberTo)(player.atk)}（${(0, combat_stats_1.formatAtkBreakdown)(stats)}）`,
            `会心率：${stats.critRate}%${stats.defenseRate > 0 ? `，减伤率：${Math.floor(stats.defenseRate * 100)}%` : ''}`,
        ];
