...
function stoneCultivateInsufficient(amount, stone) {
    return [
        '灵石不足，无法修炼。',
        `当前灵石：${(0, utils_1.formatAmount)(stone)}枚`,
        `本次需要：${(0, utils_1.formatAmount)(amount)}枚（可获得约 ${Math.floor(amount / STONE_CULTIVATE_RATIO)} 修为）`,
        stoneCultivateUsage(),
    ].join('\n');
}
/** 双修每日次数（内存计数，按日期重置） */
    ...
    ctx.command('xiuxian/灵石修炼 <amount:integer>', '消耗灵石直接转化修为')
        ...
        if (exp <= 0) {
            return [
                ...
                `当前输入：${amount}，当前灵石：${(0, utils_1.formatAmount)(stone)}枚`,
                stoneCultivateUsage(),
            ].join('\n');
        }
        if (exp >= userGetExpMax) {
