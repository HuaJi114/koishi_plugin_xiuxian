...
function applyBoss(ctx, _config) {
    ...
    ctx.command('xiuxian/讨伐世界boss [id:integer]', '讨伐世界BOSS')
        ...
        if (bossHp <= 0) {
            ...
            return `道友对${boss.name}造成${(0, utils_1.numberTo)(dmg)}伤害，将其击杀！\n${rewardMsg}`;
        }
        await ctx.database.set('xiuxian_boss', { id: boss.id }, { hp: bossHp });
        return `道友对${boss.name}造成${(0, utils_1.numberTo)(dmg)}伤害，BOSS剩余气血${(0, utils_1.numberTo)(bossHp)}；BOSS反击造成${(0, utils_1.numberTo)(bossDmg)}伤害，道友剩余气血${(0, utils_1.numberTo)(newHp)}。`;
    });
}
