...
function formatBankLevelTable() {
    const lines = ['会员等级一览（发送【灵庄升级会员】升级）：'];
    for (let i = 1; i <= MAX_BANK_LEVEL; i++) {
        const lv = BANK_LEVEL[i];
        const up = lv.levelup ? `升级需${(0, utils_1.formatAmount)(lv.levelup)}灵石` : '已满级';
        lines.push(`${i}.${lv.level} — 存储上限${(0, utils_1.formatAmount)(lv.savemax)}，时息${(lv.interest * 100).toFixed(2)}%，${up}`);
    }
    return lines.join('\n');
}
/** 灵庄（银行）模块 */
function applyBank(ctx, _config) {
    ...
    ctx.command('xiuxian/灵庄存灵石 <amount:integer>', '存入灵石')
        ...
        if (stone < amount)
            return `道友所拥有的灵石为${(0, utils_1.formatAmount)(stone)}枚，金额不足，请重新输入！`;
        const info = await getBankInfo(session.userId);
        const lv = BANK_LEVEL[info.bankLevel];
        const nowMax = lv.savemax - info.saveStone;
        if (amount > nowMax) {
            return `道友当前为${lv.level}，可存储上限${(0, utils_1.formatAmount)(lv.savemax)}枚，已存${(0, utils_1.formatAmount)(info.saveStone)}枚，还可存${(0, utils_1.formatAmount)(nowMax)}枚！`;
        }
        const [interest, hours] = settle(info);
        if (!(await srv.costStoneForUser(session.userId, amount, pf)))
            return '灵石不足！';
        ...
        return `本次结息${hours}小时，获得利息${(0, utils_1.formatAmount)(interest)}枚!\n存入灵石${(0, utils_1.formatAmount)(amount)}枚，灵庄现存${(0, utils_1.formatAmount)(info.saveStone)}枚`;
    });
    ctx.command('xiuxian/灵庄取灵石 <amount:integer>', '取出灵石')
        ...
        if (!amount || amount <= 0)
            return '请输入正确的金额！';
        const info = await getBankInfo(session.userId);
        if (info.saveStone < amount)
            return `道友当前灵庄存有灵石${(0, utils_1.formatAmount)(info.saveStone)}枚，金额不足！`;
        const [interest, hours] = settle(info);
        info.saveStone -= amount;
        await srv.gainStoneForUser(session.userId, amount + interest, session.platform);
        await ctx.database.set('xiuxian_bank', { userId: session.userId }, { saveStone: info.saveStone, saveTime: info.saveTime });
        return `本次结息${hours}小时，获得利息${(0, utils_1.formatAmount)(interest)}枚!\n取出灵石${(0, utils_1.formatAmount)(amount)}枚，灵庄现存${(0, utils_1.formatAmount)(info.saveStone)}枚!`;
    });
    ctx.command('xiuxian/灵庄升级会员', '升级灵庄会员')
        ...
        return `成功升级至${nlv.level}！消耗灵石${(0, utils_1.formatAmount)(cost)}枚，存储上限${(0, utils_1.formatAmount)(nlv.savemax)}枚，时息${(nlv.interest * 100).toFixed(2)}%`;
    });
    ctx.command('xiuxian/灵庄信息', '查询灵庄信息')
        ...
        const lines = [
            '道友的灵庄信息：',
            `已存：${(0, utils_1.formatAmount)(info.saveStone)}灵石`,
            ...
            `存储上限：${(0, utils_1.formatAmount)(lv.savemax)}枚`,
            `当前钱包灵石：${(0, utils_1.formatAmount)(stone)}枚`,
        ];
        if (info.bankLevel < MAX_BANK_LEVEL) {
            const next = BANK_LEVEL[info.bankLevel + 1];
            lines.push(`下一级：${next.level}，升级需${(0, utils_1.formatAmount)(lv.levelup)}灵石，上限${(0, utils_1.formatAmount)(next.savemax)}枚`);
        }
        return lines.join('\n');
    });
