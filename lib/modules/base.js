...
const REGISTER_PROMPT_MS = 120000;
const ROB_BATTLE_DETAIL_TTL_MS = 120000;
const robBattleRecords = [];
function pruneRobBattleRecords(now = Date.now()) {
    for (let i = robBattleRecords.length - 1; i >= 0; i--) {
        if (robBattleRecords[i].expireAt <= now)
            robBattleRecords.splice(i, 1);
    }
}
function storeRobBattleDetail(attackerId, defenderId, log) {
...
function getRobBattleDetail(userId) {
    const now = Date.now();
...
/** 建号后引导用户选择加入宗门或成为散修 */
async function promptSectChoice(session, srv) {
    await session.send((0, preset_sects_1.formatSectRegisterPrompt)());
    let choice = '';
    for (let attempt = 0; attempt < 5; attempt++) {
        choice = (await session.prompt(REGISTER_PROMPT_MS))?.trim() ?? '';
        if (!choice)
            break;
        if (choice === '加入宗门' || choice === '成为散修')
            break;
    ...
    await session.send((0, preset_sects_1.formatPresetSectList)());
    for (let attempt = 0; attempt < 5; attempt++) {
...
function applyBase(ctx, config) {
    ...
    ctx.command('xiuxian/我要修仙', '加入修仙世界')
        ...
        const [root, rootType] = (0, utils_1.generateRoot)(srv.data);
    ...
    async function doBreakthrough(userId, useDuE) {
        ...
        const result = (0, helpers_1.breakthrough)(srv.data, player.exp, rate, level);
        if (result.type === 'top')
        ...
        const percentage = (0, utils_1.randInt)(config.levelPunishmentFloor, config.levelPunishmentLimit);
    ...
    ctx.command('xiuxian/偷灵石', '尝试偷取他人灵石')
        ...
        const target = await srv.getPlayer(targetId);
        if (!target)
            return '对方未踏入修仙界，不要对杂修出手！';
        const pf = session.platform;
        const myStone = await srv.getStoneForUser(player.userId, pf);
        if (config.stealCost > myStone)
            return '道友的偷窃准备(灵石)不足，请打工之后再切格瓦拉！';
        const result = (0, utils_1.getPowerRate)(player.power, target.power);
        if (typeof result === 'string')
            return result;
        if ((0, utils_1.randInt)(0, 100) > result) {
        ...
        let got = (0, utils_1.randInt)(Math.floor(config.stealLowerLimit * targetStone), Math.floor(config.stealUpperLimit * targetStone));
        if (got > targetStone)
            got = targetStone;
        if (got <= 0)
            return `${target.userName}道友身无分文，无从下手~`;
    ...
    ctx.command('xiuxian/抢劫', '与他人决斗抢夺灵石')
        ...
        if ((0, utils_1.isHeavilyInjured)(baseTarget.exp, baseTarget.hp))
            return '对方重伤藏匿了，无法抢劫！';
        if ((0, utils_1.isHeavilyInjured)(basePlayer.exp, basePlayer.hp))
            return '重伤未愈，动弹不得！';
        const f1 = await (0, combat_stats_1.buildFighter)(srv, session.userId);
        const f2 = await (0, combat_stats_1.buildFighter)(srv, targetId);
        if (!f1 || !f2)
            return '战斗数据异常，请稍后再试！';
        const [log, victor, finalHp] = (0, utils_1.playerFight)(f1, f2);
        ...
        const pf = session.platform;
        const appendInjury = (msg, selfHp, foeHp, foeName) => {
            if (selfHp <= 0)
                return `${msg}\n${HEAVY_INJURY_HINT}`;
            if (foeHp <= 0)
        ...
        if (victor === player.userName) {
            ...
            if (foeStone > 0) {
                ...
                return appendInjury(`大战一番，战胜对手，获取灵石${robbed}枚，修为增加${exps}，对手修为减少${Math.floor(exps / 2)}\n${ROB_DETAIL_HINT}`, finalHp[player.userId], finalHp[target.userId], target.userName);
            }
            return appendInjury(`大战一番，战胜对手，结果对方是个穷光蛋，修为增加${exps}，对手修为减少${Math.floor(exps / 2)}\n${ROB_DETAIL_HINT}`, finalHp[player.userId], finalHp[target.userId], target.userName);
        }
        const myStone = await srv.getStoneForUser(player.userId, pf);
        ...
        if (myStone > 0) {
            ...
            return appendInjury(`大战一番，被对手反杀，损失灵石${lost}枚，修为减少${exps}\n${ROB_DETAIL_HINT}`, finalHp[player.userId], finalHp[target.userId], target.userName);
        }
        return appendInjury(`大战一番，被对手反杀，修为减少${exps}\n${ROB_DETAIL_HINT}`, finalHp[player.userId], finalHp[target.userId], target.userName);
    });
    ...
    ctx.command('xiuxian/排行榜 [type:string]', '查看各类排行榜')
        ...
        if (type === '灵石') {
            ...
            list.forEach((i, n) => { msg += `第${n + 1}位  ${i.userName}  灵石：${(0, utils_1.numberTo)(i.stone)}枚\n`; });
            return msg;
        }
        if (type === '战力') {
            const list = await srv.powerTop();
            let msg = '✨位面战力排行榜TOP10✨\n';
            list.forEach((i, n) => { msg += `第${n + 1}位  ${i.userName}  战力：${(0, utils_1.numberTo)(i.power)}\n`; });
            return msg;
        }
        if (type === '宗门') {
            const list = await srv.sectScaleTop();
            let msg = '✨位面宗门建设排行榜TOP10✨\n';
            list.forEach((i, n) => { msg += `第${n + 1}位  ${i.sectName}  建设度：${(0, utils_1.numberTo)(i.sectScale)}\n`; });
            return msg;
        }
        const list = await srv.realmTop();
        let msg = '✨位面境界排行榜TOP10✨\n';
        list.forEach((i, n) => { msg += `第${n + 1}位 ${i.userName} ${i.level}，修为${(0, utils_1.numberTo)(i.exp)}\n`; });
        return msg;
    });
    // GM：神秘力量（增加灵石）
    ctx.command('xiuxian/神秘力量 <amount:integer>', '【管理】赠送灵石', { authority: helpers_1.ADMIN_AUTHORITY })
        .action(async ({ session }, amount) => {
        ...
        for (const p of players)
            await srv.gainStoneForUser(p.userId, amount, p.platform);
        return `全服通告：赠送所有用户${amount}灵石，请注意查收！`;
    });
    // 重置状态
    ctx.command('xiuxian/重置状态', '【管理】重置玩家状态', { authority: helpers_1.ADMIN_AUTHORITY })
        .action(async ({ session }) => {
        const targetId = (0, helpers_1.getAtId)(session);
        if (targetId) {
    ...
    /** 校验突破 CD，返回提示信息或空 */
    function checkLevelCd(levelUpCd, cdMinutes) {
        if (!levelUpCd || levelUpCd.getTime() <= 0)
            return undefined;
        const diff = (0, utils_1.dateDiffSeconds)(new Date(), levelUpCd);
