...
function applyBase(ctx, config) {
    ...
    ctx.command('xiuxian/抢劫', '与他人决斗抢夺灵石')
        ...
        if (!f1 || !f2)
            return '战斗数据异常，请稍后再试！';
        const [log, victor, finalHp] = (0, utils_1.playerFight)(f1, f2, srv.data);
        ...
        if (victor === player.userName) {
            ...
            if (foeStone > 0) {
                ...
                return appendInjury(`大战一番，战胜对手，获取灵石${robbed}枚，修为增加${exps}，对手修为减少${Math.floor(exps / 2)}\n${battle_detail_1.BATTLE_DETAIL_HINT}`, finalHp[player.userId], finalHp[target.userId], target.userName);
            }
    ...
    ctx.command('xiuxian/查看战斗详情', '查看最近一场战斗过程（抢灵石/讨伐BOSS）')
        ...
        const record = (0, battle_detail_1.getBattleDetail)(userId);
        if (!record)
