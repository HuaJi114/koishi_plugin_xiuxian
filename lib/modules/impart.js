...
function applyImpart(ctx, config) {
    const srv = ctx.xiuxian;
    const MAX_ATK_PRACTICE = 9;
    function practiceCost(nextLevel) {
        return nextLevel * 500000;
    }
    async function doUpgrade(userId, platform) {
        ...
        if (!(await srv.costStoneForUser(userId, cost, platform))) {
            return `参悟传承需消耗灵石${(0, utils_1.formatAmount)(cost)}枚，道友灵石不足！`;
        }
        await srv.addAtkPractice(userId, 1);
        await srv.updatePower(userId);
        const bonusPct = (0, utils_1.formatAmount)(next * 4);
    ...
    ctx.command('xiuxian/传承帮助', '传承系统帮助')
        ...
        `3、第 n 重消耗灵石 n×500000（例：第1重 ${(0, utils_1.formatAmount)(practiceCost(1))}，第9重 ${(0, utils_1.formatAmount)(practiceCost(9))}）。`,
    ].join('\n'));
