...
exports.EXERCISE_LEVELS = void 0;
exports.applyExercises = applyExercises;
const node_fs_1 = require("node:fs");
const node_path_1 = require("node:path");
const utils_1 = require("../utils");
const DATA_ROOT = (0, node_path_1.join)(__dirname, '..', '..', 'data', 'xiuxian');
function loadExerciseLevels() {
    const raw = JSON.parse((0, node_fs_1.readFileSync)((0, node_path_1.join)(DATA_ROOT, '炼体境界.json'), 'utf-8'));
    const levels = [];
    for (const [name, cfg] of Object.entries(raw)) {
        if (cfg.cost_stone === undefined)
            continue;
        levels.push({
            name,
...
exports.EXERCISE_LEVELS = EXERCISE_LEVELS;
/** 炼体系统 */
function applyExercises(ctx, _config) {
    ...
    ctx.command('xiuxian/炼体查看', '查看炼体状态')
        ...
        const lines = [
            ...
            `炼体加成：攻击+${(0, utils_1.formatAmount)(ex.atkBuff)} 防御+${(0, utils_1.formatAmount)(ex.defBuff)} 暴击+${(0, utils_1.formatAmount)(ex.critBuff)} 暴伤+${(0, utils_1.formatAmount)(ex.critDmgBuff)}`,
        ];
        if (next) {
            lines.push(`下一层：${next.name}，消耗灵石${(0, utils_1.formatAmount)(next.costStone)}，成功率${next.probability}%`);
        }
        else {
            lines.push('已达最高炼体境界！');
        }
    ...
    ctx.command('xiuxian/炼体', '消耗灵石进行炼体突破')
        ...
        const next = EXERCISE_LEVELS[ex.exercisesLevel];
        if (!next)
            return '道友炼体已臻化境，无需再炼！';
        if (!(await srv.costStoneForUser(userId, next.costStone, pf))) {
            return `炼体需消耗灵石${(0, utils_1.formatAmount)(next.costStone)}枚，道友灵石不足！`;
        }
        if ((0, utils_1.randInt)(1, 100) > next.probability) {
            return `道友炼体失败，灵石已消耗${(0, utils_1.formatAmount)(next.costStone)}枚，莫要气馁再试！`;
        }
        ...
        return `恭喜道友炼体成功，晋升【${next.name}】！攻击+${(0, utils_1.formatAmount)(next.atkBuff)} 防御+${(0, utils_1.formatAmount)(next.defBuff)}`;
    });
}
