...
function guildOf(session) {
    return session.guildId ?? undefined;
}
/** 坊市模块（按群独立） */
function applyShop(ctx, config) {
    ...
    ctx.setInterval(async () => {
        ...
        for (const guildId of guildIds) {
            const pick = (0, utils_1.randChoice)(SHOP_AUTO_ITEMS);
    ...
    ctx.command('xiuxian/坊市查看', '查看本群坊市')
        ...
        const lines = ['☆------本群坊市------☆'];
        items.forEach((it) => {
            lines.push(`${it.id}、${it.goodsName}（${it.goodsType}）${(0, utils_1.formatAmount)(it.price)}灵石 — 卖家：${it.sellerName}`);
        });
        return lines.join('\n');
    });
    ctx.command('xiuxian/坊市购买 <num:integer>', '购买坊市物品')
        ...
        if (!(await srv.costStoneForUser(userId, item.price, session.platform))) {
            return `购买需要灵石${(0, utils_1.formatAmount)(item.price)}枚，道友灵石不足！`;
        }
        ...
        return `道友购得【${item.goodsName}】，花费灵石${(0, utils_1.formatAmount)(item.price)}枚！`;
    });
    ctx.command('xiuxian/坊市上架 <name:string> <price:integer>', '上架背包物品')
        ...
        return `【${back.goodsName}】已上架坊市，标价${(0, utils_1.formatAmount)(price)}灵石！`;
    });
    ...
    ctx.command('xiuxian/系统坊市上架 <name:string> <price:integer>', '【管理】上架任意物品', { authority: helpers_1.ADMIN_AUTHORITY })
        .action(async ({ session }, name, price) => {
        const gid = guildOf(session);
        if (!gid)
            return '坊市仅在群聊中使用！';
        ...
        await ctx.database.create('xiuxian_shop', {
            ...
            goodsId: Number(id),
            goodsName: info.name,
            goodsType: info.item_type ?? '物品',
            price,
            goodsNum: 1,
            createTime: new Date(),
        });
        return `系统已上架【${info.name}】，标价${(0, utils_1.formatAmount)(price)}灵石！`;
    });
}
